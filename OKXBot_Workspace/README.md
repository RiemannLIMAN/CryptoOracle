...
*   **右脑 (感知)**：集成 **DeepSeek-V3** 大模型，不仅能读懂 K 线形态（如头肩顶、收敛三角），更能结合宏观逻辑进行趋势推演。
...
*   🦁 **激进趋势猎手 (High Trend)**：当市场单边剧烈波动时，AI 变身为激进派，放大止盈空间，紧咬主升浪。
...
### 5. 💠 终端可视化看板 (CLI Dashboard)

*   **ASCII 收益曲线**：启动时自动绘制最近 5 次交易的盈亏柱状图 (🟩/🟥)，直观展示战绩。
...
2.  **AI 动态买卖标准**：DeepSeek 在不同市场形态下的具体决策逻辑表。
3.  **交易执行的三重门**：价格时效性、最小交易额与风控取小逻辑。
