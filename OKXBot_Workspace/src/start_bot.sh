...
if [ -n "$VENV_PATH" ]; then
    ...
    if [ -n "$CONDA_PREFIX" ]; then
        ...
        # 只要您配置了 CUSTOM_VENV_NAME="okx_ds"，哪怕没激活，也尝试用 conda run 跑
        if [ -n "$CUSTOM_VENV_NAME" ] && command -v conda &> /dev/null; then
            # 检查该环境是否存在
            if conda env list | grep -q "$CUSTOM_VENV_NAME"; then
                echo -e "${GREEN}✅ 检测到 Conda 环境 '$CUSTOM_VENV_NAME' (未激活)，将使用 conda run 运行...${NC}"
                # 注意：这里我们构造一个特殊的命令前缀
                PYTHON_CMD="conda run -n $CUSTOM_VENV_NAME --no-capture-output python"
            else
                echo -e "${YELLOW}⚠️ 未找到名为 '$CUSTOM_VENV_NAME' 的 Conda 环境${NC}"
            fi
        fi
...
nohup $PYTHON_CMD -u okx_deepseek.py > "$STARTUP_LOG" 2>&1 &
...
if ps -p $NEW_PID > /dev/null; then
    ...
    echo "--------------------------------------------------"
    echo "可能的原因:"
    echo "1. 依赖库未安装 (运行 pip install -r requirements.txt)"
